...
function register(){
	...
	global $db, $errors, $username, $email, $name, $postID;
	...
	if (count($errors) == 0) {
		...
		if (isset($_POST['isAdmin'])) {
			...
			$query = "INSERT INTO account (username, password, name, email, isAdmin, postID) 
					  VALUES('$username', '$password', '$name', '$email', 0, '$postID')";
			...
			$query = "INSERT INTO account (username, password, name, email, isAdmin, postID) 
			VALUES('$username', '$password', '$name', '$email', 1, '$postID')";
			mysqli_query($db, $query);
